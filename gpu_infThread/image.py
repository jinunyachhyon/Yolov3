...
def read_preprocess(image_path, read_image_queue):
    # Pre-processing
    ...
    read_image_queue.put(image)

    # print(image.shape) # ([1,3,416,416])


####### Inference
def inference(read_image_queue, batch_detections_queue):
    image = read_image_queue.get()
    with torch.no_grad():
...
def display(batch_detections_queue, image_path):
    ...
    for idx, detections in enumerate(batch_detections):
        ...
        output_path = 'prediction_thread.jpg'
...
def main():
    ...
    # Create shared queue
    read_image_queue = queue.Queue()
    batch_detections_queue = queue.Queue()

    t1 = threading.Thread(target=read_preprocess, args=(image_path, read_image_queue))
    t1.start()
    t1.join()

    t2 = threading.Thread(target=inference, args=(read_image_queue, batch_detections_queue))
    t2.start()
    t2.join()

    t3 = threading.Thread(target=display, args=(batch_detections_queue, image_path))
    t3.start()
    t3.join()
